...
description: "Detections of females and males per sex, month and area around the mouth of the Westerschelde."
execute:
  echo: false
  include: false
output:
  html_document:
    ...
    # toc_depth: 2
subtitle: analysis progress of the acoustic detections
date: 2.28.2023
editor_options:
...
summary_masterias_length <- masterias_info %>% group_by(sex) %>% summarise(mean_length = mean(length1), sd_length = sd(length1))

abs_detect_ind <- masterias_detections_clean %>% group_by(sex) %>% mutate(count = 1) %>% summarise(abs_ind = tag_serial_number %>% unique %>% length(), abs_detect = sum(count))
```
